...
const purgeOrphanDeletionLedger = `-- name: PurgeOrphanDeletionLedger :one
WITH scan AS (
    SELECT d.event_id, d.target_event_id, d.created_at
    FROM deletion_events d
    WHERE (d.created_at, d.event_id) > ($1::bigint, $2::text)
      AND d.created_at < $3::bigint
...
type PurgeOrphanDeletionLedgerParams struct {
	CursorUnix        int64
...
type PurgeOrphanDeletionLedgerRow struct {
	Scanned       int64
	Deleted       int64
	LastCreatedAt int64
	LastEventID   string
...
// cursor from (0, ”).
func (q *Queries) PurgeOrphanDeletionLedger(ctx context.Context, arg PurgeOrphanDeletionLedgerParams) (PurgeOrphanDeletionLedgerRow, error) {
	row := q.db.QueryRow(ctx, purgeOrphanDeletionLedger,
		arg.CursorUnix,
		arg.CursorEventID,
		arg.CreatedBeforeUnix,
		arg.RowLimit,
	)
	var i PurgeOrphanDeletionLedgerRow
	err := row.Scan(
		&i.Scanned,
		&i.Deleted,
		&i.LastCreatedAt,
