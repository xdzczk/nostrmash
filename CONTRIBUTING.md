...
- Recommended local toolchain: Go `1.26.6` (`go.mod` `toolchain go1.26.6`)
- CI and Docker builders pin Go `1.26.6` for reproducible checks
...
`make verify-docker` runs the same blocking gates as CI inside a pinned Go `1.26.6` container, with an isolated Postgres sidecar for integration-backed checks.
