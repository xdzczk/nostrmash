...
- `query.Service.GetThreadWindow` repeatedly called `GetThread` while paging replies.
- That re-fetched the same focal event and ancestor chain on every page instead of once per request.

How it was measured:

- Benchmark: `go test -run=^$ -bench=BenchmarkServiceGetThreadWindow -benchmem ./internal/query`
- Baseline sample before change:
  - `144991 ns/op`
  - `98057 B/op`
  - `1617 allocs/op`
...
- `GetThreadWindow` now fetches event + ancestors once, then pages only reply rows via `GetEventReplies`.
- Cursor-loop dedupe was also changed from formatted string keys to a typed struct key to avoid per-page format allocations.

What improved:

- Post-change sample:
  - `145289 ns/op` (flat within run-to-run noise)
  - `95918 B/op` (down ~2.2%)
  - `1578 allocs/op` (down ~2.4%)
- Multi-run confirmation (`-count=5`) also showed lower steady-state allocation and lower average latency trend (`136591 ns/op`, `95923 B/op`, `1578 allocs/op` in this environment).

### 2026-04: Event-tag expansion allocation pressure

What was slow:

- `store.ExpandEventTags` used append growth from zero capacity even though total output row count is known up front.
- This path sits directly on canonical ingest fan-out.

How it was measured:

- Benchmark: `go test -run=^$ -bench=BenchmarkExpandEventTags -benchmem ./internal/store`
- Baseline sample before change:
  - `11061 ns/op`
  - `27650 B/op`
  - `104 allocs/op`
...
- Added a first pass to precompute output row count and preallocate `[]model.EventTag` capacity accordingly.

What improved:

- Post-change sample:
  - `8619 ns/op` (down ~22.1%)
  - `13448 B/op` (down ~51.4%)
  - `97 allocs/op` (down ~6.7%)

Correctness note:

- Both changes keep data shape and ordering semantics unchanged; only repeated work/allocation behavior was tuned.
