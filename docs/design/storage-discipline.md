...
- `idx_event_tags_e_lookup` `(value, event_id, tag_index) WHERE tag_name='e' AND value_index=0` — owner: highlights-by-event-id in [internal/store/compat_queries_followers_highlights.go](../../internal/store/compat_queries_followers_highlights.go). **Dropped in migration 000056**: production showed `idx_scan=0` over 6+ weeks (9.3 GB) and EXPLAIN confirmed the highlights query is served by `idx_event_tags_tag_name_value` instead. `idx_trusted_note_discovery_hops_score` (13 GB for ~900k rows, `idx_scan=0`; all reads join/delete by the `event_id` primary key) was dropped in the same migration.
